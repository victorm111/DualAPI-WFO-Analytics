...
def test_read_config_file():
    ...
    try:
        ...
        if hasattr(exc, "problem_mark"):
            ...
            print(
                "**config.yaml file Error position: (%s:%s)"
        ...
        assert (
            cfgfile_parse_error == 0
        ), "assert error test_read_config_file: yaml cfg file not read"  # if cfgfile_parse_error = 1
        ...
        LOGGER.info(
            f"conftest:: test_read_config_file() python version: {python_version}"
        )
        LOGGER.info(
            f"conftest:: test_read_config_file() pytest version:  {pytest_version}"
        )
