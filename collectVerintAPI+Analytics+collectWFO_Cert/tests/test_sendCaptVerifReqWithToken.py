...
class test_CaptureVerification:
    def __init__(self, test_read_config_file: object) -> None:
        ...
        self.Payload_start_time = (
            "2023-12-18T00:00:00-00:00"  # "start_time": "2023-12-08T00:00:00-00:00",
        )
        ...
        if self.collect_yesterday:
            self.Payload_start_time = (
                self.yesterdaydate + "T00:00:00-00:00"
            )  # if 24hr yesterday data collection
        ...
        self.CaptVerifDaily_noCDRnotFound = (
            pd.DataFrame()
        )  # Capt Verif results df but with no 'Record in error, no CDR found' errors filter out, CCaaS CDR not supported
    ...
    def test_getCaptVerifCSV_buildReq(self, test_read_config_file, getVerintToken):
        ...
        LOGGER.debug(
            f"test_getCaptVerifCSV_buildreq:: build request headers: {req.headers}"
        )
        LOGGER.debug(
            f"test_getCaptVerifCSV_buildreq:: build request payload: {req.data}"
        )
    ...
    def test_getCaptVerifCSV_sendReq(self, session, preppedReq) -> object:
        ...
        try:
            self.s = self.session.send(
                preppedReq, timeout=25, verify=False
            )  # send request
            ...
            assert (
                self.s.status_code == 200
            ), "test_getCaptVerifCSV_sendReq():: session request response not 200 OK"
            # access session cookies
            # print(f'session cookies: {s.cookies}')
            LOGGER.info(
                f"test_getCaptVerifCSV_sendReq:: request start {self.Payload_start_time}"
            )
            LOGGER.info(
                f"test_getCaptVerifCSV_sendReq:: request start {self.Payload_end_time}"
            )

            LOGGER.info(
                f"test_getCaptVerifCSV_sendReq:: AWE Capt Verif csv request response received {self.s.status_code}, now attempt write Capt Verif zip archive to {self.zipPath}"
            )
            ...
            try:
                with open(self.zipPath, "wb") as zipFile:
                    ...
                    LOGGER.debug(
                        "test_getCaptVerifCSV_sendReq:: capt verif req response zip file write file OK"
                    )
            except:
                LOGGER.exception(
                    "test_getCaptVerifCSV_sendReq:: AWE Capt Verif zip archive cannot be opened"
                )
            else:
                LOGGER.info(
                    "test_getCaptVerifCSV_sendReq:: AWE Capt Verif zip retrieved OK"
                )
            ...
            try:
                ...
                LOGGER.exception(
                    "test_getCaptVerifCSV_sendReq:: AWE Capt Verif unzip exception"
                )
            else:
                LOGGER.debug(
                    "test_getCaptVerifCSV_sendReq:: AWE capt verif zip archive unzip success"
                )
            ...
            LOGGER.debug(
                f"test_getCaptVerifCSV_sendReq:: AWE Capt Verif confirm csv available after unzip"
            )

            check.not_equal(
                self.csv_file,
                [],
                "test_getCaptVerifCSV:: error csv file not found after unzip",
            )

            # read the csv into a df
            LOGGER.info(
                f"test_getCaptVerifCSV_sendReq:: AWE Capt Verif unzipped csv now available at {self.csv_file[0]}, now try to open csv and read into df"
            )
            # read csv headers
            try:
                ...
                LOGGER.exception(
                    "test_getCaptVerifCSV_sendReq:: unzip capt verif zip file failed"
                )
            else:
                reader = csv.reader(f)
                LOGGER.debug(
                    "test_getCaptVerifCSV_sendReq:: AWE Capt Verif results csv read OK"
                )
                self.csv_headers = next(reader, None)
                self.CaptVerifDaily_df = pd.read_csv(
                    self.csv_file[0], header=0, names=self.csv_headers
                )
                # delete rows if error 'Recorded in error, CDR not found' reported in row as TRUE, AWE does not support CCaaS CDR
                self.CaptVerifDaily_noCDRnotFound = self.CaptVerifDaily_df[
                    self.CaptVerifDaily_df["Recorded in error, CDR not found"] != True
                ]

                LOGGER.info(
                    f"test_getCaptVerifCSV_sendReq:: AWE Capt Verif capt verif csv, calls found with issues: {len(self.CaptVerifDaily_noCDRnotFound)} "
                )

                # dump csv with 'no CDR found' errors removed
                # determine filename of current csv

                self.csv_noCDR_Error = self.csv_file[0].replace(
                    ".csv", "_noCDR_Errors.csv"
                )

                try:
                    self.CaptVerifDaily_noCDRnotFound.to_csv(
                        self.csv_noCDR_Error, index=False, header=self.csv_headers
                    )
                except:
                    LOGGER.exception(
                        "test_getCaptVerifCSV_sendReq:: AWE Capt Verif without CDR error results csv write error"
                    )
                else:
                    LOGGER.debug(
                        "test_getCaptVerifCSV_sendReq:: AWE Capt Verif results w/o CDR errors csv written OK"
                    )
                ...
                check.equal(
                    len(self.CaptVerifDaily_noCDRnotFound),
                    0,
                    " !!!!! test_getCaptVerifCSV:: AEE Capt Verif df non zero size after csv read, call rec issues indicated !!!!!",
                )
                # check if data

                LOGGER.info(
                    f"***** test_getCaptVerifCSV_sendReq():: AWE Capt Verif API test routines finished"
                )
