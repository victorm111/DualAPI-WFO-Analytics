...
class test_ClassCollectEngID:
    ...
    """
        ...
        self.df_DetailEngDaily_sorted_NotRecorded = (
            pd.DataFrame()
        )  # records  calls missing from AWE S&R but in Analytics ED
        self.df_sorted_Recorded_notIn_DetailEngDaily = (
            pd.DataFrame()
        )  # records calls missing from Analytics ED but in AWE S&R
        ...
        self.ED_column_headers = (
            list()
        )  # Analytics ED column headers returned from API call
        self.SR_column_headers = (
            list()
        )  # Verint S&R column headers returned from API call

        self.csv_DailyMissing_output = test_read_config_file["dirs"][
            "ED_column_headers"
        ]  # calls in ED but not in AWE S&R
    ...
    """
        ...
        LOGGER.debug(
            "test_collect_df:: init test_AnalyticsEngagementDetailReport class"
        )
        ...
        (
            self.df_DetailEngDaily,
            self.AnalyticsNumber_calls,
            self.ED_column_headers,
        ) = test_DetailReport.test_AnalyticdED_sendRequest(
            self.ED_session, self.ED_prepped
        )  # retrieves daily data

        LOGGER.debug("test_collect_df:: init test_SearchReplay class")
        ...
        (
            self.SR_session,
            self.SR_prepped,
        ) = test_SRReport.test_getSearchAndReplay_buildReq(getVerintToken)
        LOGGER.debug(
            "test_collect_df:: init test_SearchReplay class complete, send request"
        )
        self.df_SR = test_SRReport.test_getSearchAndReplay_sendReq(
            self.SR_session, self.SR_prepped
        )
        ...
        (
            self.CaptVerif_session,
            self.CaptVerif_prepped,
        ) = test_CaptVerifReport.test_getCaptVerifCSV_buildReq(
            test_read_config_file, getVerintToken
        )
        (
            self.df_CaptVerificationDaily,
            self.captVerifResult,
            self.CaptVerif_session,
        ) = test_CaptVerifReport.test_getCaptVerifCSV_sendReq(
            self.CaptVerif_session, self.CaptVerif_prepped
        )
        self.number_of_tests += 1  # increment number of tests
        check.equal(
            self.captVerifResult,
            True,
            "test_getCaptVerifCSV(): AWE reported call recording issues",
        )
        ...
        self.listTest["Interval_ref"].append(
            "starting:" + test_CaptVerifReport.Payload_start_time + ","
            "ending:" + test_CaptVerifReport.Payload_end_time
        )
        self.listTest["Interval_compare"].append("n/a")
        self.listTest["ref_num_calls"].append(
            len(test_CaptVerifReport.CaptVerifDaily_noCDRnotFound)
        )
        ...
        LOGGER.debug(
            "test_collect_df:: finished collecting Analytics and Verint API data"
        )
    ...
    """
        ...
        if self.AnalyticsNumber_calls != 0:
            # sort by start times
            if len(self.df_SR):
                self.df_SR_sorted = self.df_SR.sort_values(
                    by="local_audio_start_time", ascending=False
                )
            if len(self.df_DetailEngDaily):
                self.df_DetailEngDaily_sorted = self.df_DetailEngDaily.sort_values(
                    by="dialog_start_time", ascending=False
                )
            if len(self.df_CaptVerificationDaily):
                self.df_CaptVerificationDaily_sorted = (
                    self.df_CaptVerificationDaily.sort_values(
                        by="Start time", ascending=False
                    )
                )

            LOGGER.info(
                f"test_collectDF:: test_compare_df:: Analytics ED no. calls: {len(self.df_DetailEngDaily)}, Verint S&R no. calls: {len(self.df_SR)}, Verint Capture Verif no. calls {len(self.df_CaptVerificationDaily)} "
            )
            LOGGER.debug(
                "test_collectDF:: test_compare_df:: start compare dataframes, collect Analytics Eng Detail engagement_ids"
            )
            self.dfAnalyticsED_EngIDS = self.df_DetailEngDaily.engagement_id

            LOGGER.debug(
                f"test_collectDF:: test_compare_df:: Analytics ED eng ids are: \n {self.dfAnalyticsED_EngIDS}"
            )
            ...
            LOGGER.debug(
                f"test_collectDF:: test_compare_df:: Verint S&R returned engagement ids are: \n {self.dfSR_EngIDS}"
            )
            LOGGER.info(
            ...
            try:
                ...
                self.listTest["Description"].append(
                    "Checks all call eng IDs returned from CCaaS Analyticd ED detailed report are listed in AWE S&R"
                )
                ...
                self.listTest["ref_num_calls"].append(
                    len(self.df_DetailEngDaily_sorted)
                )
                ...
                try:
                    self.df_DetailEngDaily_sorted_NotRecorded.to_csv(
                        self.csv_DailyMissing_output,
                        index=False,
                ...
                LOGGER.info(
                    "********** SUCCESS test_compare_df:: test_compare_df() NO call recording mismatch, all call eng IDs in Analytics ED rpt (as reference) are listed in AWE S&R **********"
                )
                # update test dictionary
                self.listTest["Result"].append("PASSED")
            ...
            try:
                self.listTest["TestName"].append(
                    "checkZeroAnalyticsEngIDsAlsoInAWE-S&R"
                )
                self.listTest["Description"].append(
                    "Checks zero call eng IDs returned from CCaaS Analyticd ED detailed report matched in AWE S&R"
                )
                ...
                LOGGER.error(
                    f"test_compare_df::test_compare_df() Analytics number calls = 0 but {self.SR_Number_calls} calls returned from AWE S&R"
                )
                LOGGER.debug(
                ...
                # dump the calls
                try:
                    self.df_SR_sorted.to_csv(
                        self.csv_DailyMissingED_output,
                        index=False,
        ...
        if len(self.df_SR):
            ...
            LOGGER.info(
                "test_compare_df:: ASSERTION check if all call Eng IDs listed in AWE S&R are matched to engagement ids returned in Analytics Eng Detailed Report"
            )
            ...
            try:
                # update test dictionary
                self.listTest["TestName"].append(
                    "checkAllAWE-S&RcallIDsAlsoInAnalyticsEDreport"
                )
                self.listTest["Description"].append(
                    "Checks all call eng IDs returned from AWE S&R matched in CCaaS Analyticd ED detailed report"
                )
                ...
                self.listTest["compared_num_calls"].append(
                    len(self.df_DetailEngDaily_sorted)
                )
                ...
                try:
                    self.df_sorted_Recorded_notIn_DetailEngDaily.to_csv(
                        self.csv_DailyMissingED_output,
                        index=False,
                        header=self.SR_column_headers,
                    )
                ...
                self.listTest["Result"].append("PASSED")
                LOGGER.info(
                    "********** SUCCESS test_compare_df:: ALL call eng ids listed in AWE S&R (as reference) are matched to engagement ids listed in Analytics Eng Detailed Report ****** "
                )
            ...
            try:
                # update test dictionary
                self.listTest["TestName"].append(
                    "checkZeroAWECalls-MatchedInAnalyticsEDreport"
                )
                self.listTest["Description"].append(
                    "Checks zero call eng IDs returned from AWE S&R matched in CCaaS Analyticd ED detailed report"
                )
                ...
                try:
                    self.df_DetailEngDaily_sorted.to_csv(
                        self.csv_DailyMissing_output,
                        index=False,
                        header=self.ED_column_headers,
                    )
        ...
        LOGGER.info(
            f"test_collectDF:: test_compare_df:: all API call eng ID results comparison testing finished"
        )
        LOGGER.info(
            f"test_collectDF:: test_compare_df:: Analytics reported calls: {self.AnalyticsNumber_calls}, AWE S&R calls: {self.SR_Number_calls}"
        )
        LOGGER.info(
