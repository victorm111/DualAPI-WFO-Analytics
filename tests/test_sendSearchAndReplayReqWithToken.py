...
class test_SearchReplay:
    def __init__(self, test_read_config_file: object) -> None:
        ...
        self.Payload_start_time = (
            "2023-12-18T00:00:00.000Z"  #  "beginPeriod": "2023-12-18T00:00:00.000Z",
        )
        ...
        self.base_report_folder = (
            os.getenv("ROOT_DIR") + test_read_config_file["dirs"]["SR_report"]
        )
    ...
    def test_getSearchAndReplay_buildReq(self, getVerintToken):
        ...
        LOGGER.debug(
            "test_getSearchAndReplay_buildReq:: token retrieved, build request"
        )
        ...
        LOGGER.info(
            f"test_getSearchAndReplay_buildReq:: request start: {self.Payload_start_time}"
        )
        LOGGER.info(
            f"test_getSearchAndReplay_buildReq:: request start: {self.Payload_end_time}"
        )
        self.SR_send_url = "https://" + self.URL + self.URL_api
        req = requests.Request(
            "POST",
            url=self.SR_send_url,
        ...
        LOGGER.debug(
            f"test_getSearchAndReplay_buildReq:: build request headers: {req.headers}"
        )
        LOGGER.debug(
            f"test_getSearchAndReplay_buildReq:: build request payload: {req.data}"
        )
    ...
    def test_getSearchAndReplay_sendReq(self, session, preppedReq) -> object:
        ...
        LOGGER.info(
            f'test_getSearchAndReplay_sendReq:: request interval: "starting:" + {self.Payload_start_time} "ending:" + {self.Payload_end_time}'
        )
        ...
        try:
            ...
            ), "test_getSearchAndReplay() session request response not 200 OK"
            self.s.raise_for_status()

            print(
                f"test_getSearchAndReplay() session resp received code: {self.s.status_code}"
            )
            LOGGER.info(
                f"test_getSearchAndReplay:: response received: {self.s.status_code}"
            )
            ...
            try:
                ...
                LOGGER.debug(
                    "test_getSearchAndReplay:: test_getSearchAndReplay() json unpacked ok"
                )
                check.not_equal(
                    len(self.response_dict),
                    0,
                    "test_getSearchAndReplay() empty json returned",
                )

                self.no_calls = len(self.response_dict.get("Sessions"))
                LOGGER.info(
                    f"test_getSearchAndReplay:: test_getSearchAndReplay() number of calls returned: {self.no_calls}"
                )

                if self.no_calls:
                    LOGGER.info(
                        f"test_getSearchAndReplay:: AWE test_getSearchAndReplay() calls retrieved, attempt write to csv at location: {self.csv_output}"
                    )
                    # store call data headers from the json
                    self.csv_headers = list(
                        self.response_dict.get("Sessions")[0].keys()
                    )
                    ...
                    for calls in range(self.no_calls):
                        mylist.append(
                            list(self.response_dict.get("Sessions")[calls].values())
                        )
                    ...
                    check.not_equal(
                        len(self.SR_df),
                        0,
                        "test_getSearchAndReplay() ERROR no calls df returned",
                    )
                    self.SR_df_sorted = self.SR_df.sort_values(
                        by="local_audio_start_time", ascending=False
                    )

                    try:
                        self.SR_df_sorted.to_csv(
                            self.csv_output, index=False, header=self.csv_headers
                        )
                    except:
                        LOGGER.exception(
                            "test_getSearchAndReplay::  AWE S&R csv creation error"
                        )
                    else:
                    ...
                    LOGGER.info(
                        "test_getSearchAndReplay::  no calls retrieved therefore no output csv created, finish up"
                    )
